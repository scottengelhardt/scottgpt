...
def generate_response(message):
    similar_response = st.session_state.db.similarity_search(message) # do a similarity search in our database for a matching vector
    data = [doc.page_content for doc in similar_response] # get the contents of the vector
    response = st.session_state.chain.run(message=message, data=data, history=st.session_state.history) # send the message and relevant data to our llm
...
def main():
    ...
    if "embeddings" not in st.session_state:
        ...
        st.session_state.db = Pinecone.from_existing_index("scott", st.session_state.embeddings)
    ...
    with col2:
      if st.button('Interactive Portfolio'):
          webbrowser.open_new_tab("https://share.mindstamp.com/w/RgoLuuzfyGFi?from=streamlit")
    with col3:
